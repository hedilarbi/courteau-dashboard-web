...
import { createSizeGroup } from "@/services/sizesGroupeServices";
...
const Page = () => {
  ...
  const addGroupSize = async () => {
    try {
      setIsSubmitting(true);
      const formattedName = name.trim();

      const selectedSizesIds = selectedSizes.map((size) => size._id);

      const response = await createSizeGroup(formattedName, selectedSizesIds);
      if (response.status) {
        setSuccessMessage("Groupe de tailles créé avec succès");
        setName("");
        setSelectedSizes([]);
      } else {
        setError(response.message);
